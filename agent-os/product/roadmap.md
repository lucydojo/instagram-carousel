...
2. [ ] Supabase auth + workspace foundations — Users can sign in via custom glassmorphic auth UI (Supabase Auth backend), access is invite-only for demo (allowlist + super admins), workspaces support branding (name/logo), core tables + RLS policies enforce tenant isolation with view-only sharing (members see all carousels but only edit their own). `M`
...
5. [ ] Prompt builder + optimization options — Users can define topic/prompt and structured options (tone/mood, audience, slides count) and save/load user-saveable presets (e.g., "Entrepreneur minimal style") for consistent generation. `M`
